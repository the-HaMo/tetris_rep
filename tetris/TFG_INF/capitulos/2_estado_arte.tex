...
A diferencia de la criomicroscopía electrónica convencional (cryo-EM)~\cite{delmic_cryoem_vs_cryoet}, que suele centrarse en el análisis \textit{in vitro} de partículas purificadas, la Criomicroscopía Electrónica de Tomografía (cryo-ET) permite el estudio de las muestras \textit{in situ}. Esto significa que es posible visualizar macromoléculas como complejos proteicos, filamentos y membranas biológicas dentro de su contexto celular original y a una resolución nanométrica.\\
...
\subsection{Inserción de proteínas solubles a través Self-Avoiding Worm-Like Chain (SAWLC)}
...
\begin{itemize}
    \item \textbf{Selección y rotación estocástica}: El proceso se inicia extrayendo una molécula de la base de datos de entrada y aplicándole una rotación aleatoria uniforme para asegurar que el empaquetamiento final sea isotrópico y no presente sesgos de orientación.
...
En el contexto de su integración en el \textit{framework} \textbf{PolNet}, se hace evidente que el algoritmo debe ser adaptado para adquirir consciencia del contexto estructural previo. Bajo esta premisa, la máscara de ocupación inicial debe incorporar el volumen de las densidades preexistentes, como membranas, garantizando que el mapa de correlación identifique posiciones válidas que respeten estrictamente los demás elementos biológicos. Asimismo, se requiere el diseño de un mecanismo de generación de clústeres inteligente que trascienda la dispersión aleatoria; para ello, el algoritmo debe utilizar las posiciones de máxima afinidad geométrica para iniciar de forma localizada el crecimiento de las proteínas solubles, asegurando una ocupación homogénea y compacta de la totalidad del volumen del tomograma.
