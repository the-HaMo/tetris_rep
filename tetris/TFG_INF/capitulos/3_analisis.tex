...
\begin{itemize}
    \item \textbf{Estudio comparativo de la capacidad de empaquetamiento}: Evaluar el grado de ocupación (\textit{occupancy}) alcanzado por el algoritmo \textbf{Tetris} frente al modelo \textbf{SAWLC}. Se analizará el tiempo de ejecución, el grado de ocupancia, el número de proteínas insertadas, etc.
...
\begin{enumerate}
    \item \textbf{Sistema usado:} Todas las pruebas, ejecuciones y benchmarks de este proyecto se realizaron en una estación de trabajo de escritorio con las siguientes especificaciones:
        ...
        \begin{itemize}
            ...
            \item \textbf{Versión de CUDA Toolkit:} 12.0 (la versión 13.0 está disponible, pero se optó por la 12.0 por su mayor estabilidad)
        \end{itemize}

    \item \textbf{Entorno virtual: } El desarrollo y la ejecución del código se llevaron a cabo dentro de un entorno virtual de Python (\texttt{venv}), garantizando el aislamiento de dependencias respecto al sistema y la reproducibilidad de los experimentos. Para recrear dicho entorno deben ejecutarse los siguientes comandos:

    \begin{lstlisting}[language=bash]
    python3 -m venv .venv
    source .venv/bin/activate
    pip install -r requirements.txt        # entorno CPU
    pip install -r requirements-gpu.txt   # entorno GPU (CUDA 12)
    \end{lstlisting}
    ...
    \begin{table}[H]
        ...
        \hline
        \texttt{numpy} & $\geq$ 1.24 & Operaciones con arrays multidimensionales \\
        ...
        \hline
        \texttt{cupy-cuda12x} & $\geq$ 12.0 & Operaciones GPU equivalentes a NumPy/SciPy \\
...
Esta fase se centró en adaptar el algoritmo Tetris al \textit{framework} PolNet con el objetivo de facilitar una futura integración. En particular, fue necesario modificar el algoritmo para que tuviera en cuenta estructuras biológicas preexistentes, como membranas y filamentos, durante el proceso de inserción de proteínas solubles y añadir una generación de clústeres inteligente.
...
\begin{enumerate}
    ...
    \begin{algorithm}[H]
        ...
        $tetris.output\_volume[\lnot allowed\_mask] \gets 500.0$ \tcp*{nueva etapa: $\rho_{mem} \gg \theta$}
    \end{algorithm}
    ...
    El mecanismo de crecimiento de clústeres combina \texttt{pick\_seed()} con un contador de fallos consecutivos controlado por el parámetro $T_{cluster}$ (fijado a 10). El Algoritmo~\ref{alg:cluster} describe el bucle de inserción para cada tipo de proteína:
    ...
    \end{equation}
    El valor del 8\% es un valor convencional en cryo-ET para separar la densidad proteica real del ruido de fondo en mapas de densidad electrónica. La elección del \textit{mínimo} (en lugar del máximo) garantiza que todas las proteínas tengan al menos algunos vóxeles por encima del umbral tras la binarización, evitando que las moléculas de menor intensidad queden silenciadas. Este umbral se mantiene constante y único para todas las iteraciones del algoritmo: si cada tipo de proteína usara su propio umbral, una macromolécula con umbral bajo podría solaparse geométricamente con zonas ya marcadas como ocupadas por otra de umbral mayor, produciendo fallos en la detección de colisiones.
    ...
    Donde los \textit{vóxeles ocupados} son los que superan el umbral de binarización. Por ejemplo, para la proteína \textit{5mrc\_10A} con dimensiones de caja de ($72 \times 72 \times 72$), el volumen total es de $373{,}248$ vóxeles. Al ocupar $15{,}731$ vóxeles efectivos, su ocupación interna es del $4.21\%$.
    ...
    Si la comprobación falla (el \textit{c-map} indica buen contacto pero no garantiza ausencia de solapamiento vóxel a vóxel), la posición correspondiente se penaliza con $-10^9$, descartándola como candidato. El algoritmo continúa evaluando las siguientes posiciones de mayor correlación dentro del mismo mapa, con un máximo de 50 intentos por llamada. Si ninguna posición supera todas las comprobaciones, el algoritmo devuelve \texttt{'saturated'}, indicando que no existe espacio viable en la región local analizada.
   ...
   El experimento se realizó sobre un volumen de $500\times500\times250$ vóxeles con una
    membrana preexistente cuya ocupancia es del \textbf{11.55\%}. Se empleó la proteína
...
Esta fase cierra la metodología y da paso al Capítulo de Resultados. El objetivo es comparar cuantitativamente el algoritmo Tetris adaptado frente a SAWLC bajo las mismas condiciones experimentales, ejecutadas en el entorno descrito en la Fase~1.

\begin{enumerate}
    \item \textbf{Definición del conjunto de experimentos}: Se estableció una batería de simulaciones sobre tomogramas de membrana, variando el número de tipos de proteína insertadas. En cada iteración se elimina la proteína de menor tamaño para estudiar la relación entre diversidad molecular y saturación del volumen.

    \item \textbf{Herramientas de ejecución y análisis}: Las herramientas se organizan en la carpeta \texttt{benchmark/}, que agrupa:
    \begin{itemize}
        \item \textbf{Scripts de benchmark}: ejecutan ambos algoritmos y registran las métricas clave generando \textit{logs}.
        \item \textbf{Generador de gráficas}: produce las figuras comparativas como diagramas de barras o gráficas.
    \end{itemize}

    \item \textbf{Métricas de evaluación}: Las variables dependientes registradas en cada experimento son: tiempo de ejecución ($t$), ocupancia de proteínas ($\rho_{prot}$), ocupancia total ($\rho_{total}$), throughput (monómeros/segundo), speedup ($t_{SAWLC}/t_{Tetris}$), etc.
\end{enumerate}
