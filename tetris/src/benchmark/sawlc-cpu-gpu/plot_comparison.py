...
# Membrane baseline detected in logs for Tomogram 3
MEMBRANE_LEVEL = 11.5450

def parse_detailed_log(file_path, algo_type):
    """Parse final metrics and per-type monomer details using robust regex."""
    try:
    ...
    # 1. TIME EXTRACTION (flexible regex)
...
# X-axis labels: cumulative protein names added per simulation
def get_prot_labels(results):
    """Return the list of protein names added in each simulation."""
    labels = []
    for sim in results:
...
# Use Tetris CPU labels as reference (or whichever is available)
...
def get_x(results, ref_labels):
    """Map each simulation to its X-axis position by protein name."""
...
# Unique color mapping for protein types
...
def set_prot_xaxis(ax, x_labels):
    ...
    ax.set_xlabel('Protein Types', fontsize=12, fontweight='bold')
...
# 1. TOTAL OCCUPANCY
ax0 = axs[0, 0]
ax0.axhline(y=MEMBRANE_LEVEL, color='black', linestyle=':', alpha=0.5, label='Membrane (11.5%)')
if not df_t.empty:
...
ax0.set_title('Total Occupancy Reached', fontsize=14, fontweight='bold')
ax0.set_ylabel('Total Occupancy (%)')
ax0.set_ylim(0, 65)
...
# 3. MONOMER POPULATION (STACKED BARS)
ax_bar = axs[1, 0]
width = 0.25
configs = [
    (tetris_results,     x_t,    -width),
    (tetris_gpu_results, x_tgpu,  0),
    (sawlc_results,      x_s,     width),
]

for sim_list, x_positions, offset in configs:
        ...
        for p in all_prots:
            ...
            if val > 0:
                ax_bar.bar(x, val, width, bottom=bottom,
                           color=color_map[p], edgecolor='black', lw=0.7)
                bottom += val

ax_bar.set_title('Monomer Population', fontsize=14, fontweight='bold')
ax_bar.set_ylabel('Count')
set_prot_xaxis(ax_bar, x_labels)

# Legend: protein type colours
leg_prots = ax_bar.legend(
    handles=[Line2D([0], [0], color=color_map[p], lw=6, label=p) for p in all_prots],
    title="Protein Types", loc='upper left', fontsize=7, ncol=2
)
...
ax3.set_title('Throughput (Proteins/min)', fontsize=14, fontweight='bold')
ax3.set_ylabel('Proteins / min')
...
print(f"\nDone. Image saved as: {output_img}")
